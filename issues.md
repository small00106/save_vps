...
| Low | 节点详情页命令执行状态与后端枚举不一致，完成态会被误渲染为“进行中” | 后端成功执行后将 `command_task.status` 写为 `done`，但前端仅将 `success` 识别为成功态，导致样式与状态含义错位 | 保留 |
...
| Low | 登录失败限流状态表不会回收过期来源键，长期运行可缓慢累积内存 | `loginFailureLimiter.state` 只在同来源再次失败或成功登录时覆盖/清理；若公网暴露且持续收到大量离散来源失败请求，map 会持续增长 | 降级 |
...
| - | 审计日志查询硬限 200 条且无分页 | 当前已支持 `limit` 参数并允许最多 500 条；分页仍是能力迭代项，不属于当前错误行为 | 删除 |
