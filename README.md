...
### 存储
- Agent 每 10s 上报受管文件树：默认数据根目录为 `~/data_save`，实际上传目录为 `~/data_save/files`；可通过 `CLOUDNEST_DATA_SAVE_DIR` 修改数据根目录，并通过 `--scan-dirs` 追加额外扫描目录
...
默认会在目标机上静默提示输入 `registration token` 和 `signing secret`。
脚本会自动：检测 OS/架构 → 下载 Agent 二进制 → 注册到 Master → 写入 root-only secrets/env 文件 → 创建 systemd 服务 → 启动。
...
自动化场景可以使用本地文件参数（避免命令行明文）：

```bash
curl -sSL http://master-ip:8800/install.sh | bash -s -- \
  --token-file /root/reg_token \
  --secret-file /root/signing_secret
```

> `reg_token` 读取自 Master 数据目录的 `secrets/reg_token`；`signing_secret` 读取自 `secrets/signing_secret`。

> 脚本会把签名密钥写入 `/etc/cloudnest-agent/signing_secret`（0600），并通过 `/etc/cloudnest-agent/agent.env` 提供给 systemd；不再把 secret 写入命令行或 unit 明文。
...
curl -sSL http://master-ip:8800/install.sh | bash -s -- \
  ...
  --secret-file /root/signing_secret \
  --port 8801 \
  --scan-dirs "/root/data_save/files,/srv/share"
```

> Agent 默认上传目录是 `~/data_save/files`。如需修改 Agent 本地数据根目录，可在运行环境中设置 `CLOUDNEST_DATA_SAVE_DIR=/path/to/data_save`，实际文件目录会变为 `/path/to/data_save/files`。

> `--scan-dirs` 用于追加文件树扫描目录（逗号分隔）；它不会改变上传落盘目录。systemd 安装场景可以在 `/etc/cloudnest-agent/agent.env` 里追加 `CLOUDNEST_DATA_SAVE_DIR=...` 后重启服务。

> 在节点上手动放入扫描目录的新文件，约 10 秒内会出现在节点详情页 `Files` 中。
...
# 上传到 VPS 后
./cloudnest-agent register --master http://master-ip:8800 --token-file /root/reg_token
CLOUDNEST_SIGNING_SECRET_FILE=/root/signing_secret ./cloudnest-agent run
```

Agent 配置存储在 `~/.cloudnest/agent.json`，注册后自动生成。`run` 阶段优先读取 `CLOUDNEST_SIGNING_SECRET_FILE`，其次兼容 `CLOUDNEST_SIGNING_SECRET`；缺失或仍使用旧公开默认值会直接启动失败。
...
如果你更习惯容器部署，也可以直接使用预构建镜像：

```bash
git clone https://github.com/small00106/save_vps.git
cd save_vps
cp .env.example .env
echo 'CLOUDNEST_PUBLIC_BASE_URL=https://ops.example.com' >> .env
docker compose pull
docker compose up -d
```
...
- `ghcr.io/small00106/save_vps:latest`

如果你要固定版本或回滚版本，修改 `.env` 里的 `CLOUDNEST_IMAGE_TAG` 后重新拉起即可：

```bash
# 升级到某个发布版本
sed -i 's/^CLOUDNEST_IMAGE_TAG=.*/CLOUDNEST_IMAGE_TAG=v0.1.0/' .env
docker compose pull
docker compose up -d
```

如果你想保留 Docker 路径，但改为本地源码构建镜像，使用覆盖文件：

```bash
docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build
```

Docker 默认数据目录在容器内 `/app/data`，因此首次启动生成的 secrets 路径是：
...
```bash
# 健康检查
curl -fsSL http://127.0.0.1:8800/healthz

# 读取 Agent 安装需要的 token / secret
docker compose exec cloudnest cat /app/data/secrets/reg_token
docker compose exec cloudnest cat /app/data/secrets/signing_secret
```

Docker 方式下部署 Agent：

```bash
docker compose exec -T cloudnest cat /app/data/secrets/reg_token > /root/reg_token
docker compose exec -T cloudnest cat /app/data/secrets/signing_secret > /root/signing_secret
chmod 600 /root/reg_token /root/signing_secret
curl -sSL https://ops.example.com/install.sh | bash -s -- \
  --token-file /root/reg_token \
  --secret-file /root/signing_secret
```
...
`release/build-release.sh` 用于在 Linux 构建机上生成正式发布产物；它不是通用的本地开发启动脚本。

前置条件：

- Linux
- `go` / `node` / `npm` / `bash` / `gcc` / `tar` / `sha256sum`
- amd64 主机需要额外安装 `aarch64-linux-gnu-gcc`
- arm64 主机需要额外安装 `x86_64-linux-gnu-gcc`
...
Docker 路径会读取仓库根目录的 `.env`；可直接从 `.env.example` 复制。

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CLOUDNEST_IMAGE_TAG` | `latest` | Docker Compose 使用的 GHCR 镜像标签 |
| `CLOUDNEST_LISTEN` | `0.0.0.0:8800` | 监听地址 |
| `CLOUDNEST_DB_TYPE` | `sqlite` | 数据库类型（sqlite / mysql） |
| `CLOUDNEST_DB_DSN` | 二进制：`./data/cloudnest.db`；Docker：`/app/data/cloudnest.db` | 数据库连接（SQLite 文件路径或 MySQL DSN） |
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CLOUDNEST_DATA_SAVE_DIR` | `~/data_save` | Agent 本地数据根目录；实际上传目录是 `<data_root>/files` |
| `CLOUDNEST_SIGNING_SECRET_FILE` | 空 | 签名密钥文件路径（推荐，优先级高于 `CLOUDNEST_SIGNING_SECRET`） |
| `CLOUDNEST_SIGNING_SECRET` | 空 | 签名密钥值（兼容旧方式；未设置文件时才使用） |

> 额外扫描目录通过 `register --scan-dirs`（或安装脚本 `--scan-dirs`）写入 `~/.cloudnest/agent.json`，不属于运行时环境变量。
...
├── internal/
│   ├── api/                # agent/auth/files/nodes/alerts/ping/command/terminal/admin
│   ├── audit/              # 审计日志写入与请求上下文提取
...
├── cmd/                    # register / run 命令
├── internal/
│   ├── agent/              # 主循环、注册、命令/Ping/文件回调
│   ├── reporter/           # 系统指标采集 + 文件树扫描
│   ├── server/             # HTTP 数据面、签名验证、限速
...
│   ├── i18n/               # 中英文本地化
│   ├── lib/                # 前端通用辅助代码
│   ├── pages/              # Dashboard/NodeDetail/FileBrowser/Terminal/PingTasks/Alerts/AuditLog/LoginPage/SettingsPage
│   └── utils/              # 下载等前端辅助逻辑
└── package.json

Dockerfile                  # 多阶段构建 (前端 + Agent交叉编译 + Master)
.env.example                # Docker 默认环境模板
docker-compose.yml          # Docker 主部署入口（拉 GHCR 镜像）
docker-compose.build.yml    # 本地源码构建覆盖层
install-master.sh           # Master 一键安装脚本（从 GitHub Releases 下载）
scripts/                    # 共享构建与 smoke test 脚本
...
1. **优先使用 Release + systemd 主路径** — 普通部署直接走 `install-master.sh`；只有你本来就偏好容器时再用 Docker
2. **妥善保管 Master secrets** — 默认会自动生成 `secrets/reg_token` 和 `secrets/signing_secret`；如果你改用环境变量覆盖，也请使用随机值并妥善保管
3. **修改 admin 密码** — 个人项目保留 `admin/admin` 作为初始入口；首次使用默认密码登录后会弹出一次提醒，仍建议尽快修改
4. **HTTPS** — 使用 nginx / caddy 反代 Master 并配置 SSL 证书（数据面通过 Master 代理，无 mixed content 问题）
5. **防火墙** — Master 开放 8800；Agent 8801 端口仅需对 Master IP 可达（不需要对公网开放）
6. **备份** — `install-master.sh` 默认把 SQLite 放在 `/var/lib/cloudnest/cloudnest.db`；Docker 默认在 `/app/data/cloudnest.db`；MySQL 部署请定期备份数据库
7. **公开地址** — 如果你走反代域名，请在环境文件里设置 `CLOUDNEST_PUBLIC_BASE_URL=https://your-domain`，让 Agent 安装脚本始终生成正确地址
8. **升级与回滚** — systemd 路径通过 `install-master.sh --version <tag>` 控制版本；Docker 路径通过修改 `.env` 里的 `CLOUDNEST_IMAGE_TAG` 控制版本
